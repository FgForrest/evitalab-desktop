/**
 * Modified version of https://github.com/electron/update-electron-app
 *
 * The original version doesn't support package.json in different place then `../package.json`. However, Vite with Electron
 * Forge puts in next to the `.vite` directory, which is resolved into `../../package.json`. This version uses the `app`
 * instance to find the app name and version.
 */
...
const userAgent = format('%s/%s (%s: %s)', app.getName(), app.getVersion(), os.platform(), os.arch());
